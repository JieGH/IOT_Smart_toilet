...
//        float offset = -0.3;
//
//        Temperature = a1 * (float) Voltage1 * Voltage1 + b1 * Voltage1 + c1 + offset;
        Lux = a2 * Voltage2 * Voltage2 + b2 * Voltage2 + c2;
        //Light=a*Voltage2+b;
        if(Lux < 2)
            Lux = 1;
        Delay10TCYx(20);
...
//                            break;
            ...
            if (CRCRight == 1) {
//                if (((RxPacket.Data1 < -100) || (RxPacket.Data1 > 1000)) ||
//                    ((RxPacket.Data2 < -100) || (RxPacket.Data2 > 1000))) {
//                    RB6 = 1;
//                } else
//                sprintf(Text, "Group ID=%u   Node ID=%u   Data1=%.2f   Data2=%.2f   RSSI =%u   CRCRight=%u \r\n",
...
//                sprintf(Text, "CRC equal to zero, Data rejected \r\n");
//                USARTOut(Text, strlen(Text));
            }
